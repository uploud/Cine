          ...
          {stages.map((stage, i) => (
            <ScrollReveal key={i} animation="fade-up" delay={i * 100} duration={600}>
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-6 sm:p-8 hover:border-primary/30 transition-all h-full flex flex-col">
                <div className="w-14 h-14 bg-primary text-white rounded-lg flex items-center justify-center font-[family-name:var(--font-display)] text-2xl font-black mb-4 shadow-md shadow-primary/20">
                  {stage.number}
                </div>
                <h3 className="font-[family-name:var(--font-display)] text-xl sm:text-2xl font-black text-slate-900 uppercase mb-2">
                  {stage.title}
                </h3>
                <p className="text-base sm:text-lg text-slate-700 leading-relaxed font-bold mt-auto">
                  {stage.description}
                </p>
              </div>
            </ScrollReveal>
